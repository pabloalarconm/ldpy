...
def parser(response, output):
    ## RDF parser:
    ## Input: HTTP GET from current directory
    ## Output: Container/Resource dict: {Cont/Res URI: Slug, ...}
    ...
    for subj, pred, obj in graph: # Takes all URI that represent Cont/Res URIs
        if output == "container":
            if str(obj) == "http://www.w3.org/ns/ldp#Container":
                list_url.append(str(subj))
        elif output == "resource":
            if str(obj) == "http://www.w3.org/ns/ldp#Resource":
                list_url.append(str(subj))
        else:
            sys.exit()
    ...
    if len(list_url) != 0:
        for l in list_url: 
            ...
            wordI = wordI.split('/')[0]
            wordI = wordI[::-1]
            list_names.append(wordI) # Extract Slug from URI
    else:
        pass

    dd=dict(zip(list_url,list_names))
    #dd.update( {"checking": "trial"} )
    return(dd)
